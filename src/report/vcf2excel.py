...
intronDict = {
    "GATA2": ["chr3", 128201827, 128202419],
...
synoVariants = [
    ["c.1416G>A", "chr3", "128199889", "C", "T"],
    ["c.1023C>T", "chr3", "128200782", "G", "A"],
    ["c.981G>A", "chr3", "128202739", "C", "T"],
    ["c.649C>T", "chr3", "128204792", "G", "A"],
    ["c.351C>G", "chr3", "128205090", "G", "C"],
    ["c.375G>A", "chr17", "7579312", "C", "T"],
    ["c.375G>T", "chr17", "7579312", "C", "A"],
    ["c.375G>C", "chr17", "7579312", "C", "G"],
    ["c.672G>A", "chr17", "7578177", "C", "T"],
    ["c.993G>A", "chr17", "7576853", "C", "T"],
]
        ...
        if lline[5] != "0":
            for gene, coordinates in gene_regions.items():
                if coordinates[0] == chrom:
                    if (
                        (start_pos >= int(coordinates[1]) and start_pos <= int(coordinates[2]))
                        or (end_pos >= int(coordinates[1]) and end_pos <= int(coordinates[2]))
                        or (start_pos < int(coordinates[1]) and end_pos > int(coordinates[2]))
                    ):
                        genes.append(gene)
    ...
    for cnv_line in cnsfile:
        ...
        if not (cnv[cns_header.index("cn")] == "2" and cnv[cns_header.index("cn1")] == "1"):
            ...
            outline = [
                ...
                cnv_end,
                cytoCoordString,
                float(cnv[cns_header.index("log2")]),
                float(cnv[cns_header.index("ci_hi")]),
                float(cnv[cns_header.index("ci_lo")]),
                cnv_baf,
                cnv[cns_header.index("cn")],
                int_or_na(cnv[cns_header.index("cn1")]),
                int_or_na(cnv[cns_header.index("cn2")]),
                cnv[cns_header.index("depth")],
                cnv[cns_header.index("probes")],
                cnv[cns_header.index("weight")],
                str(cnv[cns_header.index("gene")]),
            ]
...
# CNV GATK
worksheetCNV.conditional_format(
    "G43:G70", {"type": "cell", "criteria": "between", "minimum": -0.25, "maximum": 0.2, "format": redFormat}
)
worksheetCNV.conditional_format(
    "I43:I70", {"type": "cell", "criteria": "between", "minimum": -0.25, "maximum": 0.2, "format": redFormat}
)
...
row = row + 2
worksheetCNVkit.write(
    row, col, "Results per chromosome with aberrant calls", workbook.add_format({"bold": True, "font_size": 14})
)
row = row + 1
